...
func (v *FileView) Index() string { return v.index }

// Frame returns the frame name the view was initialized with.
func (v *FileView) Frame() string { return v.frame }
...
func (v *FileView) openFragments() error {
...
func (v *FileView) MaxSlice() uint64 {
	v.mu.RLock()
	defer v.mu.RUnlock()
...
func (v *FileView) FragmentPath(slice uint64) string {
	return filepath.Join(v.path, "fragments", strconv.FormatUint(slice, 10))
}

// Fragment returns a fragment in the view by slice.
func (v *FileView) Fragment(slice uint64) *Fragment {
	v.mu.RLock()
	defer v.mu.RUnlock()
	return v.fragment(slice)
}

func (v *FileView) fragment(slice uint64) *Fragment { return v.fragments[slice] }

// Fragments returns a list of all fragments in the view.
func (v *FileView) Fragments() []*Fragment {
	v.mu.Lock()
	defer v.mu.Unlock()
...
func (v *FileView) CreateFragmentIfNotExists(slice uint64) (*Fragment, error) {
	v.mu.Lock()
	defer v.mu.Unlock()
...
func (v *FileView) newFragment(path string, slice uint64) *Fragment {
...
func (v *FileView) SetBit(rowID, columnID uint64) (changed bool, err error) {
...
func (v *FileView) ClearBit(rowID, columnID uint64) (changed bool, err error) {
	slice := columnID / SliceWidth
	frag, err := v.CreateFragmentIfNotExists(slice)
	if err != nil {
		return changed, err
...
func (v *FileView) FieldValue(columnID uint64, bitDepth uint) (value uint64, exists bool, err error) {
	slice := columnID / SliceWidth
	frag, err := v.CreateFragmentIfNotExists(slice)
	if err != nil {
...
func (v *FileView) SetFieldValue(columnID uint64, bitDepth uint, value uint64) (changed bool, err error) {
	slice := columnID / SliceWidth
	frag, err := v.CreateFragmentIfNotExists(slice)
	if err != nil {
		return changed, err
...
func (v *FileView) FieldSum(filter *Bitmap, bitDepth uint) (sum, count uint64, err error) {
...
func (v *FileView) FieldRange(op pql.Token, bitDepth uint, predicate uint64) (*Bitmap, error) {
	bm := NewBitmap()
	for _, frag := range v.Fragments() {
...
func (v *FileView) FieldRangeBetween(bitDepth uint, predicateMin, predicateMax uint64) (*Bitmap, error) {
	bm := NewBitmap()
	for _, frag := range v.Fragments() {
...
func (v *FileView) SetRowAttrStore(a *AttrStore) {
	v.RowAttrStore = a
}

func (v *FileView) SetCacheType(cache string) {
	v.cacheType = cache
}
func (v *FileView) SetLogOutput(log io.Writer) {
	v.LogOutput = log

}
func (v *FileView) SetStats(c StatsClient) {
	v.stats = c
}

func (v *FileView) SetBroadcaster(b Broadcaster) {
	v.broadcaster = b
}
