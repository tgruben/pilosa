...
func (b *Bitmap) WriteTo(w io.Writer) (n int64, err error) {
	...
	for citer.Next() {
		...
		if c.n > 0 {
			...
			ew.WriteUint16(byte2, uint16(c.container_type))
	...
	for {
		...
		for hb++; hb < len(itr.c.bitmap); hb++ {
			if itr.c.bitmap[hb] != 0 {
				itr.j = int(hb<<6) + trailingZeroN(itr.c.bitmap[hb])
				return itr.peek(), false
			}
		}
...
func (itr *Iterator) peek() uint64 {
	...
	if itr.c.isArray() {
		return uint64(itr.key)<<16 | uint64(itr.c.array[itr.j])
	}
	if itr.c.isRun() {
		return uint64(itr.key)<<16 | uint64(itr.c.runs[itr.j].start+uint16(itr.k))
	}
	return uint64(itr.key)<<16 | uint64(itr.j)
}
